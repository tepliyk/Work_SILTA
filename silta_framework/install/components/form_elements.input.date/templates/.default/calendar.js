			...
			if(!$(this).hasClass('xdsoft_scroller_box'))
				...
				timeboxparent
					.on('scroll_element.xdsoft_scroller', function(event, percent)
						...
						setTimeout(function() {timebox.css('marginTop', -parseInt((timebox[0].offsetHeight-parentHeight)*percent))}, 10);
						})
					.on('resize_scroll.xdsoft_scroller', function( event, _percent, noTriggerScroll)
						...
						if(percent > 1) scroller.hide();
							...
							scroller
								.show()
								.css('height',parseInt(sh>10?sh:10));
							...
							if(noTriggerScroll !== true)
								timeboxparent.trigger
									...
									[_percent?_percent:Math.abs(parseInt(timebox.css('marginTop')))/(height-parentHeight)]
									);
							}
						});
				...
				timeboxparent.on('touchstart', function(event) {start = pointerEventToXY(event)});
				...
				timeboxparent.on('touchend touchcancel', function(event) {start = false});
				}
	...
	$.fn.datetimepicker = function(opt)
		...
		var
			...
			createDateTimePicker = function(input)
				{
				var
					datetimepicker   = $('<div '+(options.id?'id="'+options.id+'"':'')+' '+(options.style?'style="'+options.style+'"':'')+' class="xdsoft_datetimepicker xdsoft_'+options.theme+' xdsoft_noselect '+(options.weeks?' xdsoft_showweeks':'')+options.className+'"></div>'),
				...
				mounth_picker
					...
					.on('mousedown.xdsoft', function(event)
						...
						for(var items = select.find('div.xdsoft_option'), i = 0;i<items.length;i++)
							{
							if(items.eq(i).data('value') == val) break;
					...
					.on('mousedown.xdsoft', '.xdsoft_option', function(event)
						...
						if(_xdsoft_datetime&&_xdsoft_datetime.currentTime)
							_xdsoft_datetime
								.currentTime[$(this)
								.parent().parent()
								.hasClass('xdsoft_monthselect')?'setMonth':'setFullYear']($(this).data('value'));
							
						$(this).parent().parent().hide();
						datetimepicker.trigger('xchange.xdsoft');
						options
							.onChangeMonth&&options
							.onChangeMonth.call&&options
							.onChangeMonth.call(datetimepicker, _xdsoft_datetime.currentTime, datetimepicker.data('input'));
				...
				datetimepicker.setOptions = function(_options)
					...
					if(_options.allowTimes && $.isArray(_options.allowTimes) && _options.allowTimes.length) options['allowTimes'] = $.extend(true, [], _options.allowTimes);
					...
					if(isNaN(options.dayOfWeekStart)) options.dayOfWeekStart = 0;
					else                              options.dayOfWeekStart = parseInt(options.dayOfWeekStart)%7;
								...
								input.on('keydown.xdsoft', function(event)
									...
									switch(true)
										...
										case((key >= KEY0 && key <= KEY9) || (key >= _KEY0 && key <= _KEY9)) || (key == BACKSPACE || key == DEL):
											...
											else                       input.trigger('error_input.xdsoft');
											break;

										case(!!~([AKEY,CKEY,VKEY,ZKEY,YKEY].indexOf(key)) && ctrlDown):
										case !!~([ESC,ARROWUP,ARROWDOWN,ARROWLEFT,ARROWRIGHT,F5,CTRLKEY,TAB,ENTER].indexOf(key)): return true;
										}
									event.preventDefault();
									return false;
									});
				...
				scroll_element.append(timebox).xdsoftScroller();

				datetimepicker
					.on('afterOpen.xdsoft', function() {scroll_element.xdsoftScroller()})
					.append(datepicker)
					.append(timepicker);
				...
				var _xdsoft_datetime = new function()
					...
					_this.empty          = function()      {_this.currentTime = null};
					...
					_this.str = function() {return _this.currentTime.dateFormat(options.format)};
				...
				mounth_picker
					...
					.on('mousedown.xdsoft',function()
						...
						(function arguments_callee1(v)
							...
							if($this.hasClass(options.next))      _xdsoft_datetime.nextMonth();
							else if($this.hasClass(options.prev)) _xdsoft_datetime.prevMonth();
							if(options.monthChangeSpinner)        !stop&&(timer = setTimeout(arguments_callee1, v?v:100));
							})(500);
				...
				timepicker
					...
					.on('mousedown.xdsoft', function()
						...
						(function arguments_callee4(v)
							...
							if($this.hasClass(options.next) && (height-pheight)- options.timeHeightInTimePicker >= top) timebox.css('marginTop', '-'+(top+options.timeHeightInTimePicker)+'px')
							else if($this.hasClass(options.prev) && top-options.timeHeightInTimePicker >= 0)            timebox.css('marginTop', '-'+(top-options.timeHeightInTimePicker)+'px')
					...
					.on('xchange.xdsoft', function(event)
						...
						xchangeTimer = setTimeout(function()
							...
							while
								...
								if(options.beforeShowDay && options.beforeShowDay.call) customDateSettings = options.beforeShowDay.call(datetimepicker, start);
								else                                                    customDateSettings = null;
								if
									(
								...
								if(newRow)
									...
									table += '<tr>';
									newRow = false;
									if(options.weeks) table += '<th>'+w+'</th>';
									}

								table +=
									'<td data-date="'+d+'" data-month="'+m+'" data-year="'+y+'"'+' class="xdsoft_date xdsoft_day_of_week'+start.getDay()+' '+ classes.join(' ')+'">'+
										'<div>'+d+'</div>'+
									'</td>';
								...
								line_time = function line_time(h, m)
									...
									if
										...
										&&
											...
											options.step > 59
											||
											Math[options.roundTime](_xdsoft_datetime.currentTime.getMinutes()/options.step)*options.step == parseInt(m)
											)
										)
										{
										if(options.defaultSelect || datetimepicker.data('changed')) classes.push('xdsoft_current');
										else if(options.initTime)                                   classes.push('xdsoft_init_time');
										}
									if(parseInt(today.getHours()) == parseInt(h) && parseInt(today.getMinutes()) == parseInt(m))
										classes.push('xdsoft_today');
									time +=
										'<div class="xdsoft_time '+classes.join(' ')+'" data-hour="'+h+'" data-minute="'+m+'">'+
